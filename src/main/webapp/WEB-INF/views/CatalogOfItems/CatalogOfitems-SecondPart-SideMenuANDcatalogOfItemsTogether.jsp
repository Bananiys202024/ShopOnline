...
<li class="sku-item" id="IdForRemove${prodInfo.code}">
<div >
<div>
...
Добавить в корзину</button></a></div></div>
</div></div></div></div>
</c:if>
...
<div class="shop-add-to-cart-button">
<div data-reactroot="">
<%-- ${pageContext.request.contextPath}/shoppingListRemoveProductFromView?code=${prodInfo.code} --%>
<a>

<button id="actionbyclickremoveitem${prodInfo.code}" type="button" class="btn btn-primary btn-sm btn-block btn-leading-ficon add-to-cart-button">
<i class="ficon-cart" aria-hidden="true">

<%-- <img class="CartForItem" src="${pageContext.request.contextPath}/resources/images/CatalogOfItems/CartForItem.png" alt="cart" /> --%>

</i>Удалить</button></a></div></div>
...
Редактировать</button></a></div></div>
...
<input value="${prodInfo.id}" id="getIdItem${prodInfo.id}" style="display:none;"/>
<input value="${prodInfo.code}" id="getCodeItem${prodInfo.code}" style="display:none;"/>

<input value="" style="display:none" id="ItForCheckremoveItem${prodInfo.code}"/>
<script>



$( "#actionbyclickremoveitem${prodInfo.code}" ).click(function(e) {
	
	var code= $('#getCodeItem${prodInfo.code}').val();
	var id= $('#getIdItem${prodInfo.id}').val();
	

	$.when(executeAJAXDeleteItem(code,id)).done(function(x){
		
		if(x.msg == "Вы успешно удалили продукт"){
			$("#IdForRemove${prodInfo.code}").addClass( "HideElement" );
		}
...
function executeAJAXDeleteItem(code,id) {

	return $.ajax({
         type: "POST",
         dataType : 'json',
    contentType: "application/x-www-form-urlencoded; charset=UTF-8", // this is the default value, so it's optional
         url: "shoppingListRemoveProductFromView",
