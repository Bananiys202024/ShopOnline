...
<c:if test = "${fn:contains(ChoosenPrice, listPrice.key)}">checked = "checked"</c:if>
 onchange="$('#formPriceMenu').submit();" type = "checkbox" name = "priceSorting" value = "${listPrice.key}"/>
...
<%-- <a href="CatalogOfitems?brand=${brandSort}&priceSorting=${ChoosenPrice}TransitTypeSorting=${AttributeTransitTypeSorting}&categorySort=${VariableForSideMenucategorySort}"> --%>
<label class="container">
<input class="checkboxes"
<%-- onchange="window.location.href='CatalogOfitems?brand=${brandSort}&priceSorting=${ChoosenPrice}TransitTypeSorting=${AttributeTransitTypeSorting}&categorySort=${VariableForSideMenucategorySort}'" --%>
<c:if test = "${fn:contains(brandSort, listBrands.value)}">checked = "checked"</c:if>
 onchange="$('#formBrandMenu').submit();" type = "checkbox" name = "brand" value = "${listBrands.value}"  />
...
<input class="checkboxes"
<%-- onchange="window.location.href='CatalogOfitems?brand=${brandSort}&priceSorting=${ChoosenPrice}TransitTypeSorting=${AttributeTransitTypeSorting}&categorySort=${VariableForSideMenucategorySort}'" --%>
<c:if test = "${fn:contains(ChoosenRating, listRatings.value)}">checked = "checked"</c:if>
 onchange="$('#formRatingMenu').submit();" type = "checkbox" name = "ratingSorting" value = "${listRatings.value}"  />
<span class="checkmark"></span>
<span class="Stars">
...
<span class="item-count" tabindex="-1" aria-live="polite">9 items</span>
...
<form:form  modelAttribute="SortingVariables" action="${pageContext.request.contextPath}/CatalogOfitems" method="GET" id="formSelectMenu" accept-charset="utf-8" >
